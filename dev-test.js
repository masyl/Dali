...
with(QUnit) {
	...
	test("Instantiate template", function() {
		var template;
		template = "<div id='test'><div>{out data.ok /}</div></div>";
		$(template).dali();
		equals($.dali("test").source, "<div>{out data.ok /}</div>", "Instantiate using a jQuery selector");

		template = "<div>{out data.ok /}</div>";
	...
	test("Rendering templates inside templates", function() {
		$.dali.add("parseList", "{each data}{out this /}, {/each}");
	...
	test("Simple expressions", function() {
		$("<div id='test'><div>{out data.ok /}</div></div>").dali();
	...
	test("each Statement", function() {
		$("<div id='test'><div>{each data.fruits }{out this /} {/each}</div></div>").dali();
		equals($.dali("test").render(sampleData), "<div>Orange Banana Apple </div>");

		$("<div id='test'><div>{each data.loremItems }{out this.label /}={each this.referenceNumbers}{out this /},{/each} - {/each}</div></div>").dali();
	...
	test("if Statement", function() {
		$("<div id='test'><div>{if data.alwaysTrue }True{/if}{if data.alwaysFalse}False{/if}</div></div>").dali();
	...
	test("Text manipulation filters", function() {
		...
		$("<div id='test'><div>{out data.ok >> uppercase /}</div></div>").dali();
		equals($.dali("test").render(sampleData), "<div>ALL OK!</div>", "uppercase filter");
		$("<div id='test'><div>{out data.ok >> lowercase /}</div></div>").dali();
		equals($.dali("test").render(sampleData), "<div>all ok!</div>", "lowercase filter");

		$("<div id='test'><div>{out data.okWithWhitespaces >> trim /}</div></div>").dali();
		equals($.dali("test").render(sampleData), "<div>All Ok!</div>", "trim filter");

		$("<div id='test'><div>{if data.alwaysTrue >> uppercase}True{/if}{if data.alwaysFalse}False{/if}</div></div>").dali();
		equals($.dali("test").render(sampleData), "<div>TRUE</div>", "uppercase filter inside if statement");

		$("<div id='test'><div>{if data.alwaysTrue >> uppercase}True {out}is{/out} True{/if}</div></div>").dali();
