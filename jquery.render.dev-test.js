...
with(QUnit) {
	...
	test("Instantiate template", function() {
		...
		$(template).templates();
		equals($.templates("test").source, "<div>{out data.ok /}</div>", "Instantiate using a jQuery selector");
	...
	test("Rendering templates inside templates", function() {
		$.templates.add("parseList", "{each data}{out this /}, {/each}");
	...
	test("Simple expressions", function() {
		$("<div id='test'><div>{out data.ok /}</div></div>").templates();
	...
	test("each Statement", function() {
		$("<div id='test'><div>{each data.fruits }{out this /} {/each}</div></div>").templates();
		equals($.templates("test").render(sampleData), "<div>Orange Banana Apple </div>");

		$("<div id='test'><div>{each data.loremItems }{out this.label /}={each this.referenceNumbers}{out this /},{/each} - {/each}</div></div>").templates();
	...
	test("if Statement", function() {
		$("<div id='test'><div>{if data.alwaysTrue }True{/if}{if data.alwaysFalse}False{/if}</div></div>").templates();
	...
	test("Raw and unchanged output", function() {
		...
		$("<div id='test'>Just some text\n with no\n templating and some line feeds!</div>").templates();
